/**
    ...
    @brief Simulating the behaviour of a PIOC. Receives data via a Sampled
...
#define CLIENTID "jp3252533"
#define MQTT_SERVER "tom.uib.es"
#define TOPIC "TFG_jprodriguez/Maqueta/PIOC"
#define QOS 1

#define NUM_SAMPLES 1000000
#define SAMPLES_FILE "/home/joanpau/libiec61850-1.5/examples/TFG_PIOC_goose_p_sv_s/muestreo_ta_NM_PIOC.csv"
...
static const float max_curr = 70.0;
/* ASDU Data Variables */
static float IA, IB, IC, IN;
...
int main(int argc, char **argv)
{
    ...
    /* Using interface from virtual witch to enable communication with CSWI node */
    publisherHP = GoosePublisher_create(&gooseCommParameters, "tap1");

    /* Creating a second Publisher for messages with less priority*/
    gooseCommParameters.vlanPriority = 5;
    publisherLP = GoosePublisher_create(&gooseCommParameters, "tap1");

    if (publisherHP && publisherLP)
    {
        /* Set up the GOOSE Publisher parameters */
        GoosePublisher_setGoCbRef(publisherHP, pioc_gcb);
        GoosePublisher_setGoCbRef(publisherLP, pioc_gcb);
        GoosePublisher_setConfRev(publisherHP, 1);
        GoosePublisher_setConfRev(publisherLP, 1);
        GoosePublisher_setDataSetRef(publisherHP, "IDE.PIOC/LLN0$GO.HP$PIOC");
        GoosePublisher_setDataSetRef(publisherLP, "IDE.PIOC/LLN0$GO.LP$PIOC");
...
static void svUpdateListener(SVSubscriber subscriber, void *parameter, SVSubscriber_ASDU asdu)
{
    ...
    {
        IA = SVSubscriber_ASDU_getFLOAT32(asdu, 16);
        IB = SVSubscriber_ASDU_getFLOAT32(asdu, 20);
        IC = SVSubscriber_ASDU_getFLOAT32(asdu, 24);
        IN = SVSubscriber_ASDU_getFLOAT32(asdu, 28);
    }
    else
    {
    ...
    if (IA > max_curr || IB > max_curr || IC > max_curr || IA == 0 || IB == 0 || IC == 0 || IN > 0)
    {
        hay_alarma = true;
    }
    else
    ...
    {
        if (c_parameters.prints_on == 4)
            printf("SV --> Alarm ||| IA: %f  IB: %f  IC: %f  IN: %f\n", IA, IB, IC, IN);
